...
const Table = ({
	...
	const [addAirdropModal, setAddAirdropModal] = useState(false)
	...
	return (
		<>
			{addAirdropModal && (
				<AirdropModalForm
					close={() => setAddAirdropModal(false)}
				/>
			)}
			<div className="flex p-6 pb-0 gap-4 items-start">
				<div
					onClick={() => {
						setAddAirdropModal(true);
					}}
					...
					<CgAdd className="inline h-5 w-5 mr-1" />New airdrop
				</div>
				<button
