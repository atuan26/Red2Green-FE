...
const EventDetailPopup = ({
  data: {
    event: {
      id, title, start, end, allDay, color, description
    },
    rect,
    editEvent,
    deleteEvent,
  ...
  if (rect.window.height - rect.bottom > 300)
    ...
    stylePos.bottom = -rect.relative.bottom + (rect.height + 2)
  ...
  >
    ...
    <div className='w-full flex justify-end gap-2 '>
      <button onClick={editEvent} className='btn btn-sm btn-circle border-0 bg-white hover:bg-gray-200 text-gray-600'>
        <BiPencil className='w-4 h-4' />
      </button>
      <button onClick={() => { deleteEvent(); close() }} className='btn btn-sm btn-circle border-0 bg-white hover:bg-gray-200 text-gray-600'>
        <BsTrash className='w-4 h-4' />
      </button>
      <button onClick={close} className='btn btn-sm btn-circle border-0 bg-white hover:bg-gray-200 text-gray-600'>
    ...
    <div className='mx-4 my-2'>
      ...
      <div className='flex items-center gap-2 justify-between mx-1 my-2'>
        ...
        >
          <button className='rounded-full w-3 h-3'
      ...
      <div className='my-2 border-b border-gray-200'>
        {moment(start).startOf('day').isSame(moment(end).startOf('day')) ?
          <>
            ...
            >
              <AiOutlineCalendar className='w-5 h-5 text-gray-600' />{moment(start).format("dddd, MMMM DD, YYYY")}
            </div>
            <div
              className='flex items-center gap-2  my-2 w-full'
            >
          ...
          <>
            ...
            >
              ...
              <div className='w-full'>
                {moment(start).format("dddd, MMMM DD hh:mm A")} - {moment(end).format("dddd, MMMM DD hh:mm A")}
                {' '}
                <span className='text-gray-500'>
                  ({moment(end).from(moment(start), true)})
                </span>
