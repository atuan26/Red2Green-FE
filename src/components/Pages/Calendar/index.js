...
const MyCalendar = ({ eventList, isAuthenticated, loadEvent, deleteEvent }) => {
  ...
  const [showEventModal, setShowEventModal] = useState({ show: false, initValues: {} });
  ...
  useEffect(() => {
    const checkClickInside = (event) => {
      ...
      const isPopupButton = event?.target?.className.includes("popUpButton") || event?.target?.parentElement?.className.includes("popUpButton")
      if (!isDescendant(popupRef.current, event.target) && showEventDetailPopup?.event && !isPopupButton) {
  ...
  const handleSelectEvent = useCallback((event, e) => {
    const relativePos = relativeOffsetPosition(calendarRef.current, e.target.parentElement.parentElement)
    setShowEventDetailPopup({
      event: event,
      rect: relativePos,
      editEvent: () => { setShowEventModal({ show: true, initValues: event }) },
